...
if(ui){
 const p=window.DopamineSkillProgression;p.reset();p.grantDrop('Firebolt',2,2);p.grantDrop('Whirlwind',2,4);const panel=document.getElementById('skillLabPanel');document.getElementById('skillLabBtn')?.click();
 const fire=[...document.querySelectorAll('.g3skill')].find(x=>x.querySelector('h3')?.textContent==='Firebolt'),button=fire?.querySelector('[data-slot="1"]');button?.click();
